...
namespace PasswordUtility.Security
{
	...
	public sealed class ProtectedString
	{
		...
		private ProtectedBinary passwordUtf8;
		private string plainText;

		private bool isProtected;

		private static readonly ProtectedString EmptyProtectedString = new ProtectedString();
		public static ProtectedString Empty
		{
			get { return EmptyProtectedString; }
		}
		...
		public bool IsEmpty
		{
			get
			{
				var pBin = passwordUtf8; // Local ref for thread-safety
			    if (pBin != null)
			    {
		...
		public int Length
		{
			get
			{
				...
				ProtectedBinary pBin = passwordUtf8; // Local ref for thread-safety
				if(pBin != null)
				{
					byte[] pbPlain = pBin.ReadData();
					cachedLength = StrUtil.Utf8.GetCharCount(pbPlain);
					MemUtil.ZeroByteArray(pbPlain);
				}
				else
				{
					Debug.Assert(plainText != null);
					cachedLength = plainText.Length;
				}
		...
		private void Init(bool bEnableProtection, byte[] pbUtf8)
		{
			...
			if(bEnableProtection)
				passwordUtf8 = new ProtectedBinary(true, pbUtf8);
			else
				plainText = StrUtil.Utf8.GetString(pbUtf8, 0, pbUtf8.Length);
		}
		...
		public string ReadString()
		{
			...
			plainText = str;
			passwordUtf8 = null; // Thread-safe order
		...
		public byte[] ReadUtf8()
		{
			...
			return StrUtil.Utf8.GetBytes(plainText);
		}
		...
		{
			if(bProtect == isProtected) return this;
		...
		public ProtectedString Insert(int iStart, string strInsert)
		{
			...
			// Only operate directly with strings when isProtected is
			// false, not in the case of non-null plainText, because
			...
			ProtectedString ps = new ProtectedString(isProtected, pbNew);
